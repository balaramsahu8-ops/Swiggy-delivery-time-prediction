...
def get_model_uri(model_name):
    ...
    for stage in ("Production", "Staging"):
        staged_versions = [version for version in versions if version.current_stage == stage]
        if staged_versions:
            selected_version = max(staged_versions, key=lambda version: int(version.version))
            return f"models:/{model_name}/{selected_version.version}"

    selected_version = max(versions, key=lambda version: int(version.version))
    return f"models:/{model_name}/{selected_version.version}"


# Load the preferred deployed version, falling back when Production is empty.
model_path = get_model_uri(model_name)

try:
    # Prefer the registered model when its remote artifacts are available.
    model = mlflow.sklearn.load_model(model_path)
except Exception as exc:
    print(f"Unable to load registered model ({exc}); using local model artifact.")
